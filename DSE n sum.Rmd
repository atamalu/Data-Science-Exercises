...
This exercise is typically solved using Hashmaps. However, these are not available in base R. So to start, I decided to use this as a chance to add solutions to the already-exhaustive list.
...
This works okay for smaller arrays, but will be pretty inefficient with larger ones. Importantly, the question assumes that the input array only offers <i>one</i> solution. This allows us to use a simple 'while' loop and stop when the index combination is found.
...
Now let's test the function for different lengths of combinations to get our target number of 29.
...
The last few lines can be changed to `return(combo.mat[, i])` but a full text return may help for educational purposes.
